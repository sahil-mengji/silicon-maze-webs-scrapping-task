...
async function extractBasicPokemonStats(page, company) {
	...
	// Extract required stats using DOM selectors

	const data = await page.evaluate(() => {
		// Array of features with their names and corresponding indices to be extracted
		const features = [
		...
		features.forEach((feature) => {
			...
			featureValues[feature.name] = element ? element.innerText : null; // Ensure element exists before accessing innerText
		});
